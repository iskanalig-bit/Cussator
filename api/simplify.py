"""Vercel serverless function — POST /api/simplify (Simplify action on an AI message)."""
...
# sys.path automatically the way it would be for `python simplify.py` — add it
# explicitly rather than relying on that assumption.
sys.path.insert(0, str(Path(__file__).parent))
from _common import simplify_reply  # noqa: E402
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        status, payload = simplify_reply(body)
